    ...
    async function loadUser(user) {
        ...
        // Needed for the proprietary-target "Suggested FOSS alternatives" picker
    ...
    async function filterTargetAlternatives(q) {
        ...
        // If cache is empty, fetch broad list first
        if ((!allSolutions || allSolutions.length === 0) && db) {
            try {
                ...
                console.warn("Failed to load solutions list", e);
            }
        }

        let results = (allSolutions || []).filter((s) => {
            const name = (s?.name || "").toLowerCase();
            const pkg = (s?.package_name || "").toLowerCase();

            const matches = name.includes(query) || pkg.includes(query);
            const alreadySelected = selectedTargetAlternatives.some(
                (x) => x.pkg === s.package_name,
            );

            return matches && !alreadySelected;
        });

        // Server-side fallback if local cache gave no matches
        if (results.length === 0 && db) {
            try {
                const { data, error } = await db
                    .from("solutions")
                    .select("name, package_name")
                    .or(`name.ilike.%${query}%,package_name.ilike.%${query}%`)
                    .limit(20);
                if (error) throw error;
                results = (data || []).filter(
                    (s) => !selectedTargetAlternatives.some((x) => x.pkg === s.package_name),
                );
            } catch (e) {
        ...
        if (!results.length) {
            dd.innerHTML = `<div class="px-4 py-3 text-sm text-zinc-400">No matches found.</div>`;
        ...
        dd.innerHTML = results
            .slice(0, 20)
            .map((s) => {
                const safeName = (s.name || s.package_name).replace(/'/g, "&#39;");
                return `
    <button onclick="selectTargetAlternative('${s.package_name}', '${safeName}')"
        class="w-full text-left px-4 py-3 hover:bg-zinc-700 border-b border-zinc-700/50 last:border-0">
        <div class="font-medium text-white">${s.name || s.package_name}</div>
    ...
    document.addEventListener("click", (e) => {
        ...
        if (!e.target.closest("#link-sol-search") && !e.target.closest("#sol-dropdown"))
            hide("sol-dropdown");
        if (!e.target.closest("#target-alt-search") && !e.target.closest("#target-alt-dropdown"))
            hide("target-alt-dropdown");
        if (
